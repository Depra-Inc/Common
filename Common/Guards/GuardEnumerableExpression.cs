...
{
    public readonly struct GuardEnumerableExpression<T>
    {
        private readonly IEnumerable<T> _this;

        public GuardEnumerableExpression(IEnumerable<T> @this) => _this = @this;
        ...
        public void Empty() => When(_this, x => !x.IsNullOrEmpty(),
            @throw: x => $"{x.ToList().AsString()} should be empty.");
        ...
        private static GuardExpression<IEnumerable<T>> Common(IEnumerable<T> x) => 
            new GuardExpression<IEnumerable<T>>(x);
    }
}